...
def test_with_full_board():
    ...
    if in_check:
        ...
        for pos, piece in board.find_pieces(PieceColor.POLICE):
            ...
            if moves:
                print(
                    f"  {piece.type.value} at {pos.file}{pos.rank}: {[(m.file, m.rank) for m in moves[:5]]}{'...' if len(moves) > 5 else ''}")
...
def test_specific_scenario():
    ...
    print("Hidden king e2 pawn at e4, enemy d7 pawn at d5")

    # The mafia pawn at d5 attacks e4 from d5
    enemy_moves = get_valid_moves(board, Position("d", 5))
    print(
        f"Enemy pawn d5 can move to: {[(m.file, m.rank) for m in enemy_moves]}")
    ...
    print(
        f"Hidden king e4 can move to: {[(m.file, m.rank) for m in king_moves]}")
